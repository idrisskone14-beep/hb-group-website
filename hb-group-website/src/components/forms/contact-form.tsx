...
export function ContactForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const link = buildMailtoLink({
      subject: subject || `Message depuis le site — ${name || "Visiteur"}`,
      lines: [
        ["Nom complet", name],
        ["E-mail", email],
        ["Téléphone", phone],
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-ink-100 bg-white p-10 text-center shadow-[var(--shadow-soft)]">
        ...
        <p className="mt-2 max-w-sm text-sm text-ink-500">
          Il ne vous reste plus qu&apos;à valider l&apos;envoi pour que notre équipe reçoive votre
          message.
        </p>
        <button
          type="button"
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-ink-700">Sujet</label>
        <input
          name="subject"
          type="text"
          className="focus-ring w-full rounded-xl border border-ink-200 px-4 py-2.5 text-sm text-ink-800 placeholder:text-ink-300"
